...
// import { Float, Text } from '@react-three/drei' // 暂时注释掉未使用的导入
...
const WarpGrid: React.FC = () => {
  ...
  const fragmentShader = `
    ...
    void main() {
      ...
      // 三种颜色循环 - 针对浅色模式优化
      vec3 colors[3];
      colors[0] = vec3(0.0, 0.2, 1.0);   // 更蓝的深蓝色
      colors[1] = vec3(0.6, 0.0, 0.7);   // 深紫色  
      colors[2] = vec3(0.0, 0.6, 0.2);   // 深绿色
      ...
      if(currentWaveIndex < 1.0) {
        currentWaveColor = colors[0]; // 蓝色
      } else if(currentWaveIndex < 2.0) {
        currentWaveColor = colors[1]; // 紫色
      } else {
        currentWaveColor = colors[2]; // 绿色
      }
...
// 主导出组件 - 作为Hero背景
export const SpaceWarpAnimation: React.FC<{ className?: string }> = ({ className }) => {
