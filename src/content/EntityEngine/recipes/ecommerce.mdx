...
export const ProductModel: IEntityModel = {
  ...
  fields: {
    ...
    inventory: {
      type: 'relation',
      relation: { type: 'one_to_one', model: 'Inventory', foreignKey: 'neuronId' }
    },
    variants: {
      type: 'relation',
      relation: { type: 'one_to_many', model: 'ProductVariant', foreignKey: 'neuronId' }
    },
    reviews: {
      type: 'relation',
      relation: { type: 'one_to_many', model: 'Review', foreignKey: 'neuronId' }
    },
    seoTitle: { type: 'string' },
    seoDescription: { type: 'string' },
    seoKeywords: { type: 'string' },
...
class CartManagerActionHandler implements IActionHandler {
  ...
  private async addToCart(input: any, entityManager: IEntityManager): Promise<ActionResult> {
    ...
    // 获取产品信息
    const product = await entityManager.get('Product', input.neuronId);
    if (!product || product.status !== 'ACTIVE') {
      throw new Error('Product not available');
    }
    
    // 检查库存
    const inventory = await this.checkInventory(input.neuronId, input.variantId, input.quantity, entityManager);
  ...
  async validate(input: any): Promise<ValidationResult> {
    ...
    if (input.action === 'add') {
      if (!input.neuronId) errors.push('neuronId is required for add action');
