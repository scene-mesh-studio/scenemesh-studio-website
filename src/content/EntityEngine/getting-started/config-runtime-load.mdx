...
export class TenantConfigLoader {
  ...
  async loadTenantConfig(spaceId: string): Promise<{ models: IEntityModel[], views: IEntityView[] }> {
    // 检查缓存
    if (this.configCache.has(spaceId)) {
      return this.configCache.get(spaceId)!;
    }

    // 获取租户配置
    const tenantConfig = await this.getTenantConfig(spaceId);
    ...
    for (const modelName of tenantConfig.enabledModels) {
      try {
        ...
        console.warn(`Failed to load model ${modelName} for tenant ${spaceId}:`, error);
      }
    }

    const result = { models, views };
    this.configCache.set(spaceId, result);
    
    return result;
  }

  private async getTenantConfig(spaceId: string): Promise<TenantConfig> {
    // 从数据库或配置服务获取租户配置
    // 这里使用模拟数据
    ...
    return configs[spaceId] || { spaceId, enabledModels: ['User'] };
  }
...
const handler = async (req: Request, { params }: { params: { spaceId: string } }) => {
  ...
  try {
    // 加载租户配置
    const config = await tenantConfigLoader.loadTenantConfig(spaceId);
    ...
    return fetchEntityEntranceHandler({
      request: req,
      endpoint: `/api/${spaceId}/ee`,
      initializer
    });
    
  } catch (error) {
    console.error(`Failed to load config for tenant ${spaceId}:`, error);
